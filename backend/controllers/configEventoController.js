...
module.exports = {
  /*  
    ...
    */
  async addConfigEvent(fecha_inicio, fecha_final, hora_inicio, hora_final, id_equipo, descripcion, lugar, titulo, dia_repetible) {
    ...
    try {
      ...
      const response = await connectionPostgres.query(sqlConfig, [fecha_inicio, fecha_final, hora_inicio, hora_final, id_equipo, descripcion, lugar, titulo, dia_repetible]);
      ...
      const responseFechas = await connectionPostgres.query(fechas, [fecha_inicio, fecha_final, dia_repetible]);
      ...
      try {
        if (responseFechas.rows.length === 0) {
          ...
          return {
            statusCode: 400,
            data: "",
            message: "No se encontraron fechas en las que se repetirá el evento",
          };
        }
        ...
        const valueInserts = responseFechas.rows
          .map((fecha, index) => {
            ...
            values.push(fecha.gs, id_equipo, descripcion, hora_inicio, hora_final, titulo, lugar, id_config);
            return `($${offset + 1}, $${offset + 2}, $${offset + 3}, $${offset + 4}, $${offset + 5}, $${offset + 6}, '${estados.activo}' , $${offset + 7},$${offset + 8})`;
          })
          .join(", ");
      ...
      return {
        statusCode: 201,
        data: id_config,
        message: "Configuración de evento creada con éxito",
      };
    } catch (e) {
      console.log("Error: ", e);
      await connectionPostgres.query("ROLLBACK");
  ...
  async editConfigEvento(id, fecha_inicio, fecha_final, hora_inicio, hora_final, id_equipo, descripcion, lugar, titulo, dia_repetible) {
    try {
      ...
      const responseFechas = await connectionPostgres.query(queryFechas, [id_config]);
      /* Si la fecha de inicio y final no cambian */
      if (
        sameDate(new Date(responseFechas.rows[0].fecha_inicio), new Date(fecha_inicio)) &&
        sameDate(new Date(responseFechas.rows[0].fecha_final), new Date(fecha_final)) &&
        ...
        const response = await connectionPostgres.query(query, [hora_inicio, hora_final, id_equipo, descripcion, lugar, titulo, dia_repetible, id_config]);
        ...
        const responseEventos = await connectionPostgres.query(queryEventos, [descripcion, hora_inicio, hora_final, titulo, lugar, id_config]);
        ...
        const responseDelete = await connectionPostgres.query(queryDelete, [id_config]);
        ...
        const responseUpdateConfig = await connectionPostgres.query(queryUpdate, [fecha_inicio, fecha_final, hora_inicio, hora_final, id_equipo, descripcion, lugar, titulo, dia_repetible, id_config]);
